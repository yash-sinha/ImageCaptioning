  ...
  while true do
    ...
    for k=1,#sents do
      ...
      if opt.dump_images == 1 then
        ...
        local mkcmd = 'mkdir vis/' .. opt.image_folder
        print(mkcmd)
        os.execute(mkcmd)
        print()
        local cmd = 'cp "' .. path.join(opt.image_root, data.infos[k].file_path) .. '" vis/' .. opt.image_folder ..'/img' .. #predictions .. '.jpg' -- bit gross
        print(cmd)
...
if opt.dump_json == 1 then
  -- dump the json
  local visloc = 'vis/' .. opt.image_folder .. '/vis.json'
  utils.write_json(visloc, split_predictions)
end
